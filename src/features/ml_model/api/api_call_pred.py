...
def calc_distance(accident_dataset, lats, longs, google_count_lat_long):
    ...
    longs_r = list(itertools.chain.from_iterable(
    ...
    new['a'] = np.sin(new['dlat'] / 2) ** 2 + np.cos(new['lat1']) * \
        np.cos(new['lat2']) * np.sin(new['dlon'] / 2) ** 2
    new['distance'] = R * \
...
def call_weatherapi(place, weatherKey, tm):
    ...
    weather_url = f"http://api.weatherstack.com/current?access_key={weatherKey}&query={place}"
    ...
    weather_data_for_model["Sunrise_Sunset"] = 0 if w_data["current"]["is_day"] == "yes" else 1
    # put json into a dataframe
    # datetime_object = datetime.datetime.strptime(tm, '%Y-%m-%dT%H:%M')
    iweather = pd.DataFrame(
        weather_data_for_model, index=[0])
